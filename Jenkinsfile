...
pipeline {
    ...
    environment {
        APP_IMAGE = "bhaveshprasad/qbshop-app"
        MIGRATION_IMAGE = "bhaveshprasad/qbshop-migration"
        IMAGE_TAG = "${BUILD_NUMBER}"
        GIT_REPO = "https://github.com/BhaveshPrasad9402/Qualibytes-Ecommerce.git"
        BRANCH = "dev"
    }
    ...
    stages {
        ...
        stage('Update Kubernetes Manifests') {
            steps {
                sh '''
                sed -i "s|image: bhaveshprasad/qbshop-app:.*|image: bhaveshprasad/qbshop-app:$IMAGE_TAG|g" kubernetes/08-qbshop-deployment.yaml

                if [ -f kubernetes/12-migration-job.yaml ]; then
                    sed -i "s|image: bhaveshprasad/qbshop-migration:.*|image: bhaveshprasad/qbshop-migration:$IMAGE_TAG|g" kubernetes/12-migration-job.yaml
                fi
                '''
            }
        }
        ...
        stage('Commit & Push Changes to GitHub') {
            steps {
                ...
                withCredentials([usernamePassword(
                    ...
                    git remote set-url origin https://$GIT_USER:$GIT_PASS@github.com/BhaveshPrasad9402/Qualibytes-Ecommerce.git
